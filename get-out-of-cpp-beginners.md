...
本ドキュメントはメンテナンス性を重視してきました。C++ 特有の話ではないですが、メンテナンス性についてもう少し具体的に考えてみたいと思います。メンテナンス性が良いとは、コードの読み手がより少ない労力や時間で必要な情報を得られることと、より少ない労力や時間で必要とするコード修正やテストを行えることだと思います。

メンテナンス性に強い影響を与えるのが関数やクラスなどの部品がより疎結合になっているかどうかだと思います。良い指標があるのか分かりませんが、例えば、公開しているメンバ変数や関数インターフェースが少なく、それを呼ぶ箇所が少ない方が疎結合でしょう。また、呼んでいる箇所の数が同じくらいだとしても、const 操作の割合が多い方が疎結合と言えるでしょう。同様に、関数の引数が少ない方が疎結合でしょうし、引数が const である割合が多い方が疎結合でしょう。より疎結合であれば、ある部分的な処理を追うときに読まなければならないコード量が減ります。

似た処理が複数の箇所に存在しているのもメンテナンス性に影響を与えます。部品化しましょう。部品化とは、複数の変数や引数をひとつのクラスにまとめたり、複数行のコードを関数として独立させたりすることです。コード量が単純に減ることで、潜在的にはコードを読む労力が減ります。部品化を行うことで、部品が増え、疎結合度が下がるかも知れませんが、それよりも、複雑な関数がすっきりする方が、隠れていた構造があらわになることで、トータルではメンテナンス性に良い影響を与えると思います。これは部品化のやりすぎも場合によっては良くないということを示しています。

制御構造がコードに埋もれるのもメンテナンス性に悪影響を与えます。`if` や `for` のブロック内に、大量のコードがあったり、その場での入れ子構造が何重にもなっている状態のことです。これは制御構造を把握しづらくさせます。たとえ 1 回しか呼ばれないとしても、部品化を検討しましょう。これも構造が分かりやすくなることと疎結合度のトレードオフがあり得ると思います。

変数、関数、クラスの名前は重要です。「名は体を表す」の諺通り、中身を読まなくてもその挙動を推測するのに役立ちます。とんちんかんな名前がついていると、読み手の誤解による混乱を招き、無意味な名前がついていることよりも悪い効果をもたらすことがあり得ます。ラムダ関数が無名で許されるのは、パッと見れば内容が分かるくらい単純なものだから、わざわざ名前をつけなくても内容の把握が簡単だからなのです。良い名前をつけましょう。品詞の使い分けについては様々な場所で言及されていますので、ここでは議論しません。
