...
似た処理が複数の箇所に存在しているのもメンテナンス性に悪影響を与えます。部品化しましょう。部品化とは、複数の変数や引数をひとつのクラスにまとめたり、複数行のコードを関数として独立させたりすることです。コード量が単純に減ることで、潜在的にはコードを読む労力が減ります。部品化を行うことで、部品が増え、疎結合度が悪化するかも知れませんが、それよりも、複雑な関数がすっきりする方が、隠れていた構造があらわになることで、トータルではメンテナンス性に良い影響を与えると思います。これは部品化をとにかくすれば良いというものでもないことを示しています。

制御構造がコードに埋もれるのもメンテナンス性に悪影響を与えます。`if` や `for` のブロック内に、大量のコードがあったり、その場での入れ子構造が何重にもなっている状態のことです。これは制御構造を把握しづらくさせます。このような場合、たとえ 1 回しか呼ばれないとしても、部品化を検討しましょう。これも構造が分かりやすくなることと疎結合度のトレードオフがあり得ると思います。
